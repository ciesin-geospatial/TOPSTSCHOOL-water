...
This means the total number of individual raster layers in this NetCDF is 4020 (5 attributes x 804 time steps/months) [@CIESIN2018].
...
wsim_gldas_anoms_def <- wsim_gldas_anoms['deficit']
print(wsim_gldas_anoms_def)
```
...
This snippet uses the `cubelyr` package. Please install it first.
:::
:::
...
wsim_gldas_anoms_6yrs <- dplyr::filter(wsim_gldas_anoms_def, time %in% keeps)

print(wsim_gldas_anoms_6yrs)
...
```{r}
wsim_gldas_anoms_tex <- wsim_gldas_anoms_6yrs[texas]
```
