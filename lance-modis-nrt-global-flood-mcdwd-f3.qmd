...
# Define headers (include your token)
headers <- c(
  Authorization = "Bearer eyJ0eXAiOiJKV1QiLCJvcmlnaW4iOiJFYXJ0aGRhdGEgTG9naW4iLCJzaWciOiJlZGxqd3RwdWJrZXlfb3BzIiwiYWxnIjoiUlMyNTYifQ.eyJ0eXBlIjoiVXNlciIsInVpZCI6ImpmbWFydGluZXo0IiwiZXhwIjoxNzE1MDU0NTk3LCJpYXQiOjE3MDk4NzA1OTcsImlzcyI6IkVhcnRoZGF0YSBMb2dpbiJ9.QX03RDGdNrHYUzIS-DXrcKQDEC0OwbsEkJPLGFHEh1sQQc-nnx4EvZEGEe7bjJB5alZqfPvpOce7ixXYHaAK0bGRVlnhpEpc_KMiL_aIEcwnF9SDaDa73e7Y5-M80cagi-gpX65clwhkJec5kmCgowHHC98aAQxzqjrqwdTy9qBPlUfhlCjwMlAMZQYrO5ScHStlJJG3-IHNBTralHnDxqi43SjGZUZ6ne-9X9SqGtdEVeRq9sqL0T4-F2iHENpFdcJyJe5C6IgoV0Jdmoi9YYum5LfWW6VtnueAQRpgWoCmCK7yqYRfYKQe_dkGFtHCGZA1jhboxLGO3LswBX3KXQ"
)
...
To observe a location closer, we can clip the data and replot.We can find vector boundaries of of any country in [geoboundaries](https://www.geoboundaries.org) API, in this case the USA. 
We will use the same method as was used in the previous case studies, **[Acquiring and Pre-Processing the WSIM-GLDAS Dataset](https://ciesin-geospatial.github.io/TOPSTSCHOOL-module-1-water/wsim-gldas-acquisition.html)** and **[WSIM-GLDAS Dataset Exploration and Visualizations](https://ciesin-geospatial.github.io/TOPSTSCHOOL-module-1-water/wsim-gldas-vis.html)**
...
We can subset the `raster_df` raster with the `area_bound` vector:
```{r}
raster_subset <- raster_df[area_bound]
```

We redo the same process as above to plot the data with a basemap:
```{r}
#generate bonding box
bbox <-  sf::st_bbox(raster_subset)
...
# Plot basemap and the subset NRT Flood data
...
tm_plot <- tmap::tm_shape(bm_m)+
  tmap::tm_raster()+
  #create a new tmap shape for the NRT flood raster with style as "cat," meaning categorical.
  tmap::tm_shape(raster_subset, style="cat")+
  #add the classification styling to the raster
  tmap::tm_raster( palette = c(colors),
  title = title, 
  breaks = class_breaks,
