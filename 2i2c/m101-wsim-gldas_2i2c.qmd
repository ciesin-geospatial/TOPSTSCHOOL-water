...
First we will connect to our TOPS-SCHOOL cloud storage "bucket" to download the WSIM-GLDAS file. If you were working on a local system you would use the SEDAC website to download the files.

```{r}
# read in the 12 month integration WSIM-GLDAS file with stars
aws.s3::save_object(
  object = "water-module/composite_12mo.nc",
  bucket = "s3://tops-school",
  region = "us-west-2",
  file = "data/composite_12mo.nc"
)
```

Reading the file with the *argument* `proxy = TRUE` allows you to inspect the basic elements of the file without reading the whole file into memory. Multidimensional raster datasets can be extremely large and bring your computing environment to a halt if you have memory limitations.

```{r}
wsim_gldas <- stars::read_stars("data/composite_12mo.nc", proxy = TRUE)
...
The WSIM-GLDAS data is quite large with many variables available. We can manage this large file by selecting a single variable; in this case "deficit" (drought). Now we'll read the data back in; this time with `proxy = FALSE` and only selecting the deficit layer.
...
Again we'll connect to the SCHOOL cloud bucket and download the 1 month integration file.
...
aws.s3::save_object(
  object = "water-module/composite_1mo.nc",
  bucket = "s3://tops-school",
  region = "us-west-2",
  file = "data/composite_1mo.nc"
)
```

The structure is the same as the 12-month integration so we will go ahead and load in just the deficit layer and check the object with `print`.
...
Download the GPW tif from the cloud storage.

```{r}
# terra needs the file locally
aws.s3::save_object(
  object = "water-module/gpw_v4_population_density_rev11_2015_15_min.tif",
  bucket = "s3://tops-school",
  region = "us-west-2",
  file = "data/gpw_v4_population_density_rev11_2015_15_min.tif")
```
