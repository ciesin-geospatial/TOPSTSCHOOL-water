nyc_ui <- 
  bslib::nav_panel(
    ...
    fluidPage(
      ...
      h1(style = "padding-top: 50px; color: #3e3f3a; font-style: font-family: Roboto",
         "Lead Testing in NYS School Drinking Water - Overview"),
        bslib::card(
          fluidRow(
            shiny::markdown(
              glue::glue(
  ...
  bslib::card(
    fluidRow(shiny::markdown(
      glue::glue(
        ...
        We need to extract the coordinates (40.74779141700003, -73.74551716499997) and store the value on each side of the comma as a separate field. 
  ...
  # third ui element----
h2(style = "color: #3e3f3a;  font-style: font-family: Roboto",
       "Exploring Census County Demographics"),
  bslib::card(
    fluidRow(
