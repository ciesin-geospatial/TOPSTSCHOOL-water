...
wsim_gldas <- aws.s3::s3read_using(FUN = stars::read_stars, 
                                   proxy = FALSE, 
                                   sub = 'deficit',
...
:::{.callout-note}
## Knowledge Check 
1. What are the two ways we reduced our memory footprint when we loaded the WSIM-GLDAS data set for this lesson? (select all that apply)
...
4. The output maps of Annual Mean Deficit Anomalies for the CONUSA indicate that…
...
wsim_gldas_1mo <- aws.s3::s3read_using(
...
We can explore the data further by creating a frequency distribution (also called a histogram) of the deficit anomalies for any given spatial extent; here we are still looking at the deficit anomalies in California. We start by extracting the data from the raster time series and then create a data frame of values that are easier to manipulate into a histogram. [R data frames](https://www.w3schools.com/r/r_data_frames.asp) are data displayed in table format, which can be plotted on graphs or charts.
...
:::{.callout-note}
## Knowledge Check 
5. What's another term for ‘histogram’?
    a. Choropleth
...
```{r include = FALSE}
# you have to save the nc file to disk first if you want use terra::sds
aws.s3::save_object(
  object = "water-module/composite_1mo.nc",
  bucket = "s3://tops-school",
  region = "us-west-2",
...
1. What can zonal statistics be used for?
    ...
    c. Creating summary statistics from the values of cells that lie within a boundary.
    d. All of the above.
:::
...
```{r include=FALSE}
# clean up the files we downloaded onto the 2i2c server
file.remove(c("data/gpw_v4_population_density_rev11_2015_15_min.tif",
              "data/composite_1mo.nc",
              "data/composite_12mo.nc"))
```

:::{.callout-note}
## Knowledge Check 
1. Gridded population datasets…(select all that apply) 
