...
The `basemap_stars()` function from the `stars` package allows us to access Esri imagery layers. To generate a basemap that shows the location of our raster, we must first can define the bounding box with the `raster_df`. Choose "world_imagery" as our background and assign it to the object `bm_m`. the `st_rgb` function combines the basemap RGB image into a single file. Finally, `st_transform` transform the CRS into WGS84.
...
In this lesson we explored the LANCE MODIS Near-Rear-Time (NRT) Flood data set. In our next lesson we will think of water at the local level and focus on New York State School Drinking Water data sets.

[Lesson 3: New York Sate School Water Quality: Exposure to Lead](https://topstschool.shinyapps.io/nyc-lead/){.btn .btn-primary .btn role="button"}
