...
In this lesson, you will acquire water security data (WSIM-GLDAS) from the NASA SEDAC website. You will work with 1-month and 12-month precipitation composite datasets to extract and visualize precipitation deficit data through different plotting methods, and identify states that experienced drought. You will also work with population data to perform zonal statistics.

::: column-margin
::: {.callout-tip style="color: #5a7a2b;"}
## Coding Review

This lesson uses the [`ggplot`](https://ggplot2.tidyverse.org/) and [`terra`](https://rspatial.org/pkg/) packages. If you'd like to learn more about the functions used in this lesson you can use the help guides on their package websites.
:::
:::
...
-   Perform data exploration such as making histograms and a time series.

-   Plot geospatial data to determine precipitation deficit patterns.

    -   Use different plotting functions to make these maps.

-   Integrate across geoBoundaries, WSIM-GLDAS, and Gridded Population of the World to perform more complex analyses.
...
The Missouri River Basin functions for drinking water, irrigation and industrial purposes, and power production, while the Mississippi River benefits industries that rely on the river for trade and transport, which is important for the region's economy. Since both rivers are managed by difference offices, it was hard to determine a water resources solution that wouldn't impact both rivers.
...
[exactextractr](https://github.com/isciences/exactextractr) [@isciences2022] is an R package that summarizes raster values over groupings, or zones, also known as zonal statistics. Zonal statistics help in assessing the statistical characteristics of a certain region. The package can be used to sum all the cells in a polygon, i.e. a region; other functions include computing the mean, median, and other types of statistics.
...
Now plot the data in ggplot. Add a title and label the axis.
I have some existing code I can pull to help with the plotting--or at least make it fancy.
