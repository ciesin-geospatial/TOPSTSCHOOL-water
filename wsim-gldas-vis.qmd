...
wsim_deficit_usa |>
  ...
  plot(reset = FALSE,
    ...
    '#FFEDA3',
    # -5 to -3
    '#FFFFFF'), breaks = c(-60, -40, -20, 0, 20, 40, 50))
```
...
extracted_sum <- exact_extract(gpw1, usa, 'sum', weights = 'area')
print(extracted_sum)

extracted_sum_df <- as.data.frame(extracted_sum)
print(extracted_sum_df)

extracted_sum_df <- dplyr::mutate(extracted_sum_df, ID = dplyr::row_number())
print(extracted_sum_df)
```
...
ggplot2::ggplot(data = extracted_sum_df, ggplot2::aes(x = ID, y = extracted_sum)) + ggplot2::geom_line()


```
```{r}
tp <- terra::extract(gpw1, usa, sum, na.rm = FALSE)
print(tp)
#ggplot2::ggplot(extracted_df, ggplot2::aes(x = time, y = deficit)) + ggplot2::geom_line()
