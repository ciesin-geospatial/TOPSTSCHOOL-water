...
```{r}
#coords <- c(43.965876, -102.193077)
#coords <- c(-102.193077, 43.965876)
#Coordinates for Austin Texas
coords <- c(-97.7431, 30.2672)

#Create a stars object with point coordinates
#point_stars <- st_as_stars(st_as_sf(st_sfc(st_point(coords), crs = st_crs(wsim_gldas_1mo))))

#point_stars <- st_as_stars(st_sfc(st_point(coords), crs = st_crs(wsim_gldas_1mo)))

#point_stars <- st_sfc(st_point(coords), crs = st_crs(wsim_gldas_1mo))
#point_stars <- st_set_dimensions(point_stars, values = list(time = seq_len(st_dimensions(wsim_gldas_1mo)$time)))

point_stars <- st_sfc(st_point(coords), crs = st_crs(wsim_gldas_1mo),
                      dim = names(st_dimensions(wsim_gldas_1mo)))
...
```{r}
extracted_vals <- st_extract(wsim_gldas_1mo, point_stars)
```
...
```{r}
extracted_df <- data.frame(extracted_vals)
print(extracted_df)
...
```{r}
ggplot(extracted_df, aes(x = time, y = deficit)) + geom_line()
